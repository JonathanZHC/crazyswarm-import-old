...
public:
    ...
    crazyswarm::Takeoff::Response& res)
  {
    ...
    double takeoff_time = ros::Time::now().toSec();

    std::ofstream fw;
    fw.open("/home/dsl25/Documents/takeoff_timestamp/test.txt");
    // store array contents to text file
    fw << "start time: " << std::fixed << std::setprecision(10) << takeoff_time << "\n";
    fw.close();
    // ---------------------------------------------------------- //
    ...
    crazyswarm::Land::Response& res)
  {
    ...
    // ---------------- open the file for writing ---------------- //
    double land_time = ros::Time::now().toSec();

    std::ofstream fw;
    fw.open("/home/dsl25/Documents/takeoff_timestamp/test.txt", std::ofstream::app);
    // store array contents to text file
    fw << "landing time: " << std::fixed << std::setprecision(10) << land_time << "\n";
    fw.close();
    // ---------------------------------------------------------- //
